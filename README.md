...
- Данные приложения находятся в `%LOCALAPPDATA%\IBStart\`: общие параметры — в `settings.json`, а метаданные каталога (избранное, история последних 20 запусков, даты запуска, теги, их цвета и сортировка) — в `catalog-state.json`. История не сохраняет параметры запуска.
