...
Дополнительные параметры передаются пользователем как текст. IBStart не создаёт отдельное хранилище логинов/паролей. Если в параметрах обнаружен `/P`, `password`, `token`, `secret` или секретное присваивание в строке `/IBConnection`, программа перед запуском запрашивает подтверждение: пароль может оказаться в открытом виде в `ibases.v8i`. В логах секретные значения маскируются, а полный операнд `/IBConnection` не записывается открытым текстом. Не передавайте пароль через ярлык.
