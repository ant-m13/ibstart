...
**IBStart (ИБ Старт)** — независимое нативное приложение Windows для просмотра списка баз, хранящегося в `ibases.v8i`, и запуска установленных клиентов 1С:Предприятие 8.3/8.5. Версия `0.1.0-alpha.1` предназначена для раннего практического тестирования: формат и интерфейс ещё могут меняться.
