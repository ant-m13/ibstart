...
- Автопоиск платформ через Program Files, реестр и настраиваемые пути, включая отдельные установки тонкого клиента; в режиме «Авто» предпочитается x64, затем x86. Командная строка строится отдельно от вызова `CreateProcessW` и корректно экранирует Unicode-аргументы.
