...
- `SECRET` — значение запрещено помещать в обычный preview, log, shortcut,
  clipboard или fixture.
...
кодироваться. JWT, password и `UC` запрещены в логах и обычном preview.
...
аналогичные значения нельзя помещать в обычный preview, log, shortcut,
clipboard, fixture или report.
