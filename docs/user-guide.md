...
Для веб-подключения, распознанного как `http://`/`https://`, команда «Предприятие» сначала запускает тонкий клиент (`1cv8c.exe`). Если в `Version` или `DefaultVersion` задана версия, сначала выбирается тонкий клиент этой версии; если его нет, запускается самый новый найденный тонкий клиент, чтобы платформа могла обновить программу просмотра. При отсутствии версии сразу выбирается самый новый клиент. Подходит и отдельная установка «1С:Предприятие — Тонкий клиент», которую IBStart находит автоматически. Если тонкий клиент вообще не установлен, адрес веб-базы, в том числе 1cfresh, открывается в браузере. «Конфигуратор» для веб-базы недоступен. «Создать ярлык» сохраняет на Рабочий стол ярлык на IBStart с идентификатором базы, но без пароля.
