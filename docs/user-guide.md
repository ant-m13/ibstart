...
Для веб-подключения, распознанного как `http://`/`https://`, команда «Предприятие» запускает установленный тонкий клиент с адресом веб-сервера. Подходит и отдельная установка «1С:Предприятие — Тонкий клиент»: IBStart находит её автоматически. «Конфигуратор» для веб-базы недоступен. «Создать ярлык» сохраняет на Рабочий стол ярлык на IBStart с идентификатором базы, но без пароля.
