...
Для веб-подключения, распознанного как `http://`/`https://`, команда «Предприятие» учитывает выбранный в `App`/`DefaultApp` тип клиента. `WebClient` открывает адрес в браузере и не требует установленной платформы — так запускаются базы 1cfresh. `ThinClient` и автоматический выбор запускают установленный тонкий клиент с адресом веб-сервера; подходит и отдельная установка «1С:Предприятие — Тонкий клиент», которую IBStart находит автоматически. «Конфигуратор» для веб-базы недоступен. «Создать ярлык» сохраняет на Рабочий стол ярлык на IBStart с идентификатором базы, но без пароля.
