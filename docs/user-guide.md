...
Поле **Поиск** фильтрует дерево без учёта регистра по имени, реквизитам базы и её тегам: серверу, строке подключения и другим пользовательским полям списка. Структурный путь группы (`Folder`) и технические поля порядка (`OrderInList`, `OrderInTree`) в обычном поиске не участвуют. Запрос `кд` находит имена с `КД`, а `cluster-01` — базы с таким сервером 1С. Каждый совпавший фрагмент имени выделяется светло-зелёным фоном и тёмно-зелёным текстом; совпадение в цветной метке тега выделяется жирным. Если обычный показ меток отключён, найденная по тегу метка всё равно кратко выводится рядом с подходящей базой. Родительские группы остаются в дереве только как путь к найденной базе и автоматически раскрываются, чтобы результат был виден. После очистки поиска восстанавливается состояние групп до начала поиска.
