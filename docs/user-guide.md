...
Меню **Вид → Простой режим** оставляет на экране только поиск и дерево баз: скрываются карточка выбранной базы, действия, теги, фильтр по тегам и сортировка. В верхнем меню остаётся только пункт **Режим → Выйти из простого режима** (также работает `Ctrl+Alt+M`); после выхода обычные элементы и команды возвращаются. Это настройка удобства для пользователя, а не механизм защиты от действий в файловой системе.
