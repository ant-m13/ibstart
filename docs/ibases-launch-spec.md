...
| --- | --- | --- |
| Файл | UTF-8 с BOM/без BOM, порядок строк и неизвестные строки сохраняются; внешнее изменение контролируется fingerprint и mutex-протоколом. | Сохранять lossless-контракт и расширять регрессионные проверки. |
| Секции | Наличие `Connect` определяет базу; без него секция считается группой. Пустой `Connect`, дубли имён/ключей/ID, отсутствующие родители и циклы диагностируются и блокируют неоднозначные изменения. | Поддерживать диагностику и расширять её при добавлении typed-полей. |
...
| Raw-параметры | Неизвестные Windows-аргументы сохраняются как raw; registry блокирует конфликтующие connection/mode-параметры, повторы reserved-параметров и невалидный `/AppArch`. | Добавить version check и расширять registry по мере typed-поддержки. |
| Безопасность | Известные секреты маскируются в логах и автоматически создаваемых diagnostics/report; значения в `ibases.v8i`, UI и явном копировании доступны пользователю. | Расширить registry и тесты так, чтобы секреты не попадали в открытом виде в логи и автоматически создаваемые diagnostics/report; fixtures должны быть синтетическими или замаскированными. |
...
3. Валидный `/AppArch` из дополнительных аргументов имеет приоритет над полем
   `AppArch`; невалидное или повторное значение диагностируется и блокирует
   запуск. Любое другое непустое значение самого поля `AppArch` также
   диагностируется и блокирует запуск, но исходное значение сохраняется,
   даже если дополнительные параметры содержат валидный `/AppArch`.
...
7. `/URL`, `/IBConnection`, `/IBConnectionString`, `/F`, `/S` и `/WS` из raw
   параметров конфликтуют с typed-подключением и блокируют запуск. `/Proxy` и
   `/NoProxy` взаимоисключающие, повтор reserved-параметра блокируется. Одиночный
   `/Execute` допускается как дополнительная команда, а `/RunShortcut`,
   `/ClearCache`, `/RegServer`, `/UnregServer` и очистка saved auth требуют
   отдельного контекста и подтверждения.
8. `/P`, `/WSP`, `-pwd`, `PPasswd`, `AccessToken`, `UC`, `SPwd`, `DBPwd` и
   ...
   log и автоматически создаваемые diagnostics/report; запуск не блокируется
...
Ниже указан план достижения полной typed-поддержки, а не перечень отсутствующих
функций. В текущем коде уже есть lossless `V8iDocument`, диагностика
неоднозначного каталога, `ConnectionSpec`, проверка конфликтующих raw-параметров
и базовый набор регрессионных fixtures. Нереализованными остаются version profile,
отдельные `LaunchRequest`/`LaunchPlan` и специализированные batch/web/mobile/OLE
операции.
