...
std::optional<Release> FetchLatestRelease(std::stop_token stop) {
  const auto response = transport::FetchLatestVersionAsset(stop);
  return response ? std::optional<Release>(ParseLatestVersionFile(*response)) : std::nullopt;
}
